...
(PARTITION BY service ORDER BY satisfaction DESC) AS satisfaction_rank FROM patients;
...
(ORDER BY COUNT(p.patient_id) DESC) AS service_rank FROM service s LEFT JOIN patients p ON s.service = p.service
