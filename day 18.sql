...
FROM patients UNION ALL SELECT staff_name AS name, 'Staff' AS type FROM staff;
...
FROM patients p JOIN service s ON p.service = s.service WHERE LOWER(s.service) IN ('surgery', 'emergency')
UNION ALL
SELECT st.staff_id AS identifier,st.staff_name AS full_name,'Staff' AS type,s.service
FROM staff st JOIN service s ON st.service = s.service
